...
| IDLE watch (poll already meets P95 < 60 s at the default 30 s interval), identity gate wired with the (polarity-inverted) auth factor, clarification record/token + mail/reply parsing, real E2E | **not started** — P0-1/P0-2 **complete: Go** ([ADR-0002](adr/0002-p0-1-gmail-imap-smtp-go.md), [ADR-0004](adr/0004-p0-2-codex-exec-session-semantics.md)); P0-3 **measured** — legitimate self-mail carries no `Authentication-Results`, gate polarity must invert, wiring blocked on the user accepting [ADR-0003](adr/0003-self-mail-carries-no-auth-results.md); clarification mail format waits on the real-device walkthrough; E2E waits on the red-line-5 quota confirmation | spec §5 |
