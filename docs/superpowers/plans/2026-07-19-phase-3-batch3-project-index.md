...
- [x] 失败测试（真实 git 临时仓库 + fake io 双轨，风格照 worktree-manager.test.ts）：
...
- [x] 四件套全绿；本计划完成记录；architecture 表将「identity gate 接线/CodexDriver/
...
两任务闭环。测试基线 25 文件 / 438 → **26 文件 / 474**（+36，471 通过 + 3 live 默认 skip）；
四件套全绿。
...
| --- | --- |
| `cad5b1c` | 本计划落盘 |
| `13ac17f` | T1 projectIndex（36 测试，真实 git + fake io 双轨；默认 io 显式 stat 跟随 symlink dirent，逃逸检查因此可达并有 3 个专门测试） |
| 本提交 | T2 收尾：trust-boundary 补句、threat-model C7 allowlist 半句证据、architecture 表 |

审查结论：T1 ✅ 零必修项。要点：D-P3B3-4 对 D-P3B3-1 返回值签名的文字修订以后者为准
（合规非偏差）；空-但-有效扫描不抛为字面正确读法；lookup 闭包结构上触不到 io（路径味
拒绝是纵深防御而非承重）；`/a/b` vs `/a/bc/x` 前缀旁路经直接计算验证拒绝。

### 移交说明（给 Phase 4 路由接线者的两条备忘）

1. worktreeManager 在执行时重新 realpath 并验证"存在且是 git 仓库"，但**不验证**
   "仍属于当前配置的 allowlist"——daemon 重建/失效 ProjectIndex 的时机与策略是
   Phase 4/daemon 批次的设计点，勿假设索引常新。
2. root 内容物的信任边界已在模块 doc 写明：多写者/共享目录不在威胁模型假设 #3 的
   保护范围内，操作者须只指向自己独占控制的目录（setup 向导文案可引用此句）。
